...
class ClientIntakeAgent:
    """Creates a VA sales brief from a new client inquiry."""
    ...
    PACKAGE_RULES = [
        ("Operations VA", 75, "Best for daily admin ownership, CRM updates, follow-ups, and client communication."),
        ("Growth VA", 50, "Best for lead response, social admin, inbox triage, and pipeline support."),
        ("Starter VA", 0, "Best for focused weekly admin help and simple repeatable workflows."),
    ]
    ...
    def _build_next_steps(self, priority: str, pain_points: List[str]) -> List[str]:
        ...
        if priority == "hot":
            steps.insert(0, "Offer two call slots within the next US business day.")
    ...
    def _build_questions(self, pain_points: List[str]) -> List[str]:
        questions = [
            ...
            "Which US time zone should the VA cover for urgent replies?",
        ]
