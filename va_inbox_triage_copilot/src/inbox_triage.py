...
class InboxTriageCopilot:
    """Classifies emails and turns them into VA action steps."""
    ...
    def _sla(self, priority: str, category: str) -> str:
        if priority == "urgent":
            return "Respond within 1 US business hour"
        if category == "new lead":
            return "Respond within 2 US business hours"
        if priority == "high":
            return "Respond same US business day"
    ...
    def _extract_tasks(self, body: str, category: str) -> List[str]:
        ...
        if not tasks:
            defaults = {
                ...
                "vendor": "Review vendor message and summarize for client approval",
